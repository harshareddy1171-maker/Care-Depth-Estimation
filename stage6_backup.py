...
print("=" * 70)
print("CARE-Depth Stage 5 - Controlled Temporal Experiment")
print("=" * 70)
...
def care_fusion(
    ...
    # Never leave an invalid current pixel
    # without a temporal fallback.
    invalid = current <= 0
...
with open(
    ...
    f.write(
        "CARE-Depth Stage 5 Temporal Experiment\n"
    )
...
if best_visualization is not None:
    ...
    plt.savefig(
        os.path.join(
            FIGURES_DIR,
            "stage5_combined_reliability.png"
        ),
        dpi=200
    )
    plt.close()
